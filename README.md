...
| `BASE` | _(unset)_ | Set by the CI workflow for `scripts/check-dco.mjs`. Leave unset locally unless mimicking CI. |
| `HEAD` | _(unset)_ | Set by the CI workflow for `scripts/check-dco.mjs`. Leave unset locally unless mimicking CI. |
| `GITHUB_ACTIONS` | _(unset)_ | Set automatically by GitHub Actions; `scripts/test-offline.mjs` uses it for CI-only behavior. Leave unset locally. |
