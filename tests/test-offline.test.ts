...
describe('test-offline plan', () => {
  it('uses unprivileged unshare on local Linux when available', () => {
  ...
  it('uses sudo unshare --net on GitHub Actions Linux', () => {
    const plan = buildOfflineTestPlan({
      platform: 'linux',
      hasUnshare: true,
      githubActions: true,
    ...
    expect(plan.mode).toBe('unshare')
    expect(plan.command).toBe('sudo')
    expect(plan.args).toEqual(['unshare', '--net', '--', 'npm', 'test'])
  })
